...
if [[ "${V11_VARIANT:-}" != "control" && "${V11_VARIANT:-}" != "balanced" ]]; then
  echo "V11_VARIANT must be control or balanced" >&2
  exit 2
fi
...
case "${V11_VARIANT}" in
  control)
    RUN_LABEL="v9_control"
    DEFAULT_SEMANTIC_PROBABILITY=0.15
    DEFAULT_GROUNDED_PROBABILITY=0.15
    ;;
  balanced)
    RUN_LABEL="v10_rates"
    DEFAULT_SEMANTIC_PROBABILITY=0.12
    DEFAULT_GROUNDED_PROBABILITY=0.10
    ;;
esac

SEMANTIC_PROMPT_PROBABILITY="${SEMANTIC_PROMPT_PROBABILITY:-${DEFAULT_SEMANTIC_PROBABILITY}}"
GROUNDED_BATCH_PROBABILITY="${GROUNDED_BATCH_PROBABILITY:-${DEFAULT_GROUNDED_PROBABILITY}}"
OUT="${OUT:-output/dreamlite_compact_v11_${RUN_LABEL}_from_scratch/${SLURM_JOB_ID:-local}}"
STOP_FILE="${TMPDIR%/}/dreamlite_v11_${RUN_LABEL}_${SLURM_JOB_ID:-local}.stop"
...
# Keep V8 image-only's main text distribution. Verified rows are used only by
# the dedicated grounded loader so they cannot alter normal caption sampling.
...
echo "Mobile-OV DreamLite V11 ${V11_VARIANT} from-scratch distillation"
echo "This run explicitly uses the content-aware representation objective."
echo "PROMPT_WEIGHTS=${PROMPT_WEIGHTS}"
echo "semantic_probability=${SEMANTIC_PROMPT_PROBABILITY} grounded_probability=${GROUNDED_BATCH_PROBABILITY}"
...
"${PYTHON_BIN}" tools/utils/gpu_heartbeat.py \
  ...
  --stop-file "${STOP_FILE}" \
  --label "dreamlite-v11-${RUN_LABEL}-${SLURM_JOB_ID:-local}" &
HEARTBEAT_PID=$!
...
"${TORCHRUN_BIN}" \
  ...
  --semantic-prompt-probability "${SEMANTIC_PROMPT_PROBABILITY}" \
  ...
  --functional-batch-size 2 \
  --functional-call-weights 1,1,1,1 \
  --grounded-functional-probability 0 \
  --grounded-batch-probability "${GROUNDED_BATCH_PROBABILITY}" \
  ...
  --training-version "v11_${V11_VARIANT}" \
