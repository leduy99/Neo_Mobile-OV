...
CONFIG="${CONFIG:-configs/mobile_ov_dreamlite_compact_v10.yaml}"
...
OUT="${OUT:-output/dreamlite_compact_v10_balanced_from_scratch/${SLURM_JOB_ID:-local}}"
...
STOP_FILE="${TMPDIR%/}/dreamlite_v10_setup_${SLURM_JOB_ID:-local}.stop"
...
# Keep V8 image-only's broad text distribution. Verified image-caption pairs
# are sampled only through the dedicated grounded loader below.
...
echo "Mobile-OV DreamLite V10 balanced grounded distillation from scratch"
echo "PROMPT_MANIFESTS=${PROMPT_MANIFESTS}"
echo "PROMPT_WEIGHTS=${PROMPT_WEIGHTS} PROMPT_NAMES=${PROMPT_NAMES}"
echo "Verified grounding: probability=${GROUNDED_BATCH_PROBABILITY:-0.10} weight=${GROUNDED_FUNCTIONAL_WEIGHT:-0.5}"
echo "Semantic curriculum probability=${SEMANTIC_PROMPT_PROBABILITY:-0.12}; functional batch=${FUNCTIONAL_BATCH_SIZE:-2}"
...
"${PYTHON_BIN}" tools/utils/gpu_heartbeat.py \
  ...
  --stop-file "${STOP_FILE}" \
  --label "dreamlite-v10-setup-${SLURM_JOB_ID:-local}" &
HEARTBEAT_PID=$!
...
"${TORCHRUN_BIN}" \
  ...
  --semantic-prompt-probability "${SEMANTIC_PROMPT_PROBABILITY:-0.12}" \
  ...
  --grounded-batch-probability "${GROUNDED_BATCH_PROBABILITY:-0.10}" \
